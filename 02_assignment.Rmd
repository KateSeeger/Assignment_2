...
1. Read about the hotel booking data, `hotels`, on the [Tidy Tuesday page](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-02-11/readme.md) it came from. There is also a link to an article from the original authors. The outcome we will be predicting is called `is_canceled`. 
  ...
  _ What are some problems that might exist with the data? You might think about how it was collected and who did the collecting.  We are only looking at resort and city hotels which brings financial barrier to people who are even booking these hotels. Also, the resort hotel is most likely one part of a larger cost for a vacation. 
...
hotels_recipe <- recipe(is_canceled ~., data = hotels_mod) %>%
  step_mutate(children = ifelse(children > 1, 1, 0), 
              babies = ifelse(babies > 1, 1, 0), 
              previous_cancellations = ifelse(previous_cancellations > 1, 1, 0)) %>%
  step_mutate(agent = ifelse(agent == "NULL", 1, 0), 
              company = ifelse(company == "NULL", 1, 0)) %>%
  step_mutate(country = fct_lump_n(country, n = 5, w = NULL, other_level = "Other")) %>%
